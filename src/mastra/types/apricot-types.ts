...
export const getFormsSchema = z.object({
  pageSize: z.number().optional().describe('Number of forms to return per page (default: 25)'),
  pageNumber: z.number().optional().describe('Page number to retrieve (default: 1)'),
  sort: z.string().optional().describe('Field to sort by (e.g., "name", "-name" for descending)'),
  filters: z.record(z.string()).optional().describe('Filters to apply (e.g., {"field_123": "value"})'),
});
